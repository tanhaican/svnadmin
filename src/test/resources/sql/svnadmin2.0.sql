...
MySQL - 5.7.19 : Database - svnadmin
*********************************************************************
*/
...
CREATE TABLE `pj_gr_auth` (
  ...
  PRIMARY KEY (`pj_id`,`res`,`gr`),
  KEY `FK_Reference_6` (`pj_id`,`gr`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `pj_gr_usr` (
  ...
  PRIMARY KEY (`pj_id`,`usr`,`gr`),
  KEY `FK_Reference_10` (`pj_id`,`gr`),
  KEY `FK_Reference_9` (`usr`),
  CONSTRAINT `FK_Reference_9` FOREIGN KEY (`usr`) REFERENCES `usr` (`usr`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;


/*Table structure for table `pj_role` */

DROP TABLE IF EXISTS `pj_role`;

CREATE TABLE `pj_role` (
...
CREATE TABLE `pj_usr` (
  ...
  PRIMARY KEY (`usr`,`pj_id`),
  KEY `FK_Reference_5` (`pj_id`),
...
insert  into `role`(`code`,`name`,`comment`) values ('sadmin','超级管理员','能查看所有项目以及拥有所有权限'),('admin','普通管理员','能创建并管理项目'),('normal','普通用户','能查看有权限的项目');
...
insert  into `usr`(`usr`,`psw`,`name`,`role`,`created_by`) values ('root','cm9vdA**',NULL,'sadmin','root');
